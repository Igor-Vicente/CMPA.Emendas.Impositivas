    ...
    <>
      ...
      <main className="mx-auto min-h-[70vh] w-full max-w-360 px-4 py-8 sm:px-6 sm:py-12 lg:px-10">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Consulte os registros da {legislatura.titulo.toLocaleLowerCase("pt-BR")}.
        </p>
        ...
        <form className="mt-7 flex max-w-2xl flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-2 sm:flex-row sm:gap-3">
          ...
          <input
            name="busca"
            defaultValue={busca}
            placeholder="Buscar por título, justificativa ou assunto"
            className="h-11 min-w-0 flex-1 rounded-xl px-4 text-sm outline-none focus:ring-2 focus:ring-[#d5e8f1]"
          />
          <button className="h-11 rounded-xl bg-[#19689b] px-5 text-sm font-semibold text-white hover:bg-[#12577f]">
            Buscar
          </button>
        </form>
        ...
        <div className="mt-7 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                ...
                {resultado.itens.map((emenda) => (
                  <tr key={emenda.id} className="hover:bg-slate-50">
                    <td className="max-w-82.5 px-5 py-4">
                      ...
                      <p className="mt-1 truncate text-xs text-slate-500">{emenda.finalidade}</p>
                    </td>
                    <td className="px-5 py-4 text-sm text-slate-600">
                      {vereadoresPorId.get(emenda.vereadorId) ?? "Não identificado"}
                    </td>
                    <td className="px-5 py-4 text-sm text-[#19689b]">{emenda.assunto}</td>
            ...
            {resultado.itens.map((emenda) => (
              <article key={emenda.id} className="p-4 sm:p-5">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <strong className="shrink-0 text-sm text-slate-900">{formatarMoeda(emenda.valorEmCentavos)}</strong>
                </div>
                <Link
                  href={comLegislatura(`/emendas/${emenda.id}`, legislatura.slug)}
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500">{emenda.finalidade}</p>
                <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-100 pt-3 text-xs">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 font-medium text-slate-600">{formatarData(emenda.dataProtocolo)}</dd>
                  </div>
                </dl>
              </article>
            ))}
          ...
          {!resultado.itens.length && (
            <p className="px-6 py-14 text-center text-sm text-slate-500">Nenhuma emenda encontrada.</p>
          )}
        </div>
        {resultado.totalPaginas > 1 && (
          <div className="mt-5 flex flex-col gap-3 text-sm sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2">
              {resultado.pagina > 1 && (
                <Link
                  className="flex-1 rounded-lg border border-slate-200 bg-white px-4 py-2 text-center sm:flex-none"
