...
import {
  carregarContextoLegislatura,
  comLegislatura,
  primeiroParametro,
} from "@/lib/legislaturas";
...
export default async function VereadorPage({ params, searchParams }: Props) {
  ...
  const { legislaturas, legislatura } = await carregarContextoLegislatura(
    primeiroParametro(query.legislatura),
  );
  ...
  const hrefPagina = (numero: number) =>
    `${comLegislatura(`/vereadores/${id}`, legislatura.slug)}&pagina=${numero}`;
      ...
      <main className="mx-auto min-h-[70vh] max-w-[1440px] px-6 py-12 lg:px-10">
        ...
        <section className="mt-10">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Áreas contempladas: {porArea.join(", ") || "nenhuma área informada"}.</p>
          </div>
          <div className="mt-6 grid gap-4">
            {emendas.itens.map((emenda) => (
              <Link key={emenda.id} href={comLegislatura(`/emendas/${emenda.id}`, legislatura.slug)} className="block rounded-2xl border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#9cc6dc] hover:shadow-md sm:p-6">
                <article>
                  <div className="flex flex-col justify-between gap-4 sm:flex-row">
                    <div>
                      <span className="rounded-full bg-[#edf5f8] px-2.5 py-1 text-xs font-medium text-[#19689b]">{emenda.assunto}</span>
                  ...
                  <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 border-t border-slate-100 pt-4 text-xs text-slate-500">
                    ...
                    <span className={emenda.aprovada ? "font-semibold text-emerald-700" : "font-semibold text-amber-700"}>{emenda.aprovada ? "Aprovada" : "Não aprovada"}</span>
                  </div>
                </article>
              </Link>
            ))}
            {!emendas.itens.length && <p className="rounded-2xl border border-slate-200 bg-white px-6 py-14 text-center text-sm text-slate-500">Nenhuma emenda cadastrada para este vereador nesta legislatura.</p>}
          </div>

          {emendas.totalPaginas > 1 && (
            <div className="mt-5 flex items-center justify-between gap-3 text-sm">
              <span className="text-slate-500">Página {emendas.pagina} de {emendas.totalPaginas}</span>
              <div className="flex gap-2">
                {emendas.pagina > 1 && <Link className="rounded-lg border border-slate-200 bg-white px-4 py-2" href={hrefPagina(emendas.pagina - 1)}>Anterior</Link>}
                {emendas.pagina < emendas.totalPaginas && <Link className="rounded-lg border border-slate-200 bg-white px-4 py-2" href={hrefPagina(emendas.pagina + 1)}>Próxima</Link>}
              </div>
            </div>
          )}
        </section>
