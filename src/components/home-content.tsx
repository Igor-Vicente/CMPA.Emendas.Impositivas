...
const icones = {
  ...
  autores: (
    <>
      ...
      <path d="M3.5 20c.5-4 2.3-6 5.5-6s5 2 5.5 6M16 4.5a3 3 0 0 1 0 6M16 14c2.6.2 4 2.2 4.5 5" />
    </>
  ),
  entidades: (
    <>
      <path d="M3 21h18M5 21V8h14v13M3 8l9-5 9 5M8 12h2m4 0h2m-8 4h2m4 0h2" />
    </>
  ),
};
  ...
  return (
    <>
      <div className="mx-auto max-w-360 px-6 py-12 lg:px-10 lg:py-16">
        <section id="indicadores" aria-labelledby="titulo-indicadores">
          <div className="mb-6">
            ...
            <h2
              ...
              className="mt-2 text-2xl font-semibold tracking-tight text-[#12334d] sm:text-3xl"
            >
              Recursos da {legislatura.titulo.toLocaleLowerCase("pt-BR")}
            </h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Indicador
              titulo="Valor total destinado"
              valor={formatarMoeda(resumo.valorTotalEmCentavos)}
              detalhe="Somatório das emendas cadastradas"
              icone="valor"
            />
            <Indicador
              titulo="Total de emendas"
              valor={String(resumo.totalEmendas)}
              detalhe="Registros disponíveis para consulta"
              icone="arquivo"
            />
            <Indicador
              titulo="Autores"
              valor={String(resumo.porVereador.length)}
              detalhe="Parlamentares com emendas cadastradas"
              icone="autores"
            />
            <Indicador
              titulo="Entidades beneficiadas"
              valor={String(resumo.entidadesBeneficiadas)}
              detalhe="Beneficiários únicos contemplados"
              icone="entidades"
            />
          </div>
        </section>
        ...
        <section id="areas" className="mt-14 space-y-6" aria-labelledby="titulo-areas">
          <article className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-7">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-4">
              <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5 text-[#5275dc]" aria-hidden="true">
                <path d="M4 19h17v2H2V4h2v15Zm3-2H5v-6h2v6Zm4 0H9V7h2v10Zm4 0h-2V9h2v8Zm4 0h-2V5h2v12Z" />
              </svg>
              <h2 id="titulo-areas" className="font-semibold text-[#12334d]">
                Emendas por assunto
              </h2>
            </div>
            {assuntos.length ? (
                  ...
                  style={
                    {
                      ...
                      backgroundImage:
                        "repeating-linear-gradient(to top, transparent 0, transparent 43px, #e8edf2 44px)",
                    } as CSSProperties
                  }
                >
                  {assuntos.map((item) => (
                    <div key={item.nome} className="flex h-full flex-col justify-end text-center">
                      ...
                      />
                      <span className="mt-3 h-10 overflow-hidden text-xs leading-4 text-slate-600" title={item.nome}>
                        {item.nome}
                      </span>
                    </div>
                  ))}
          ...
          <article className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-7">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-4">
              <svg
                ...
                stroke="currentColor"
                strokeWidth="2"
                className="h-5 w-5 text-[#19689b]"
                aria-hidden="true"
              >
                <path d="M12 2v14m4-10H9.5a3 3 0 0 0 0 6h5a3 3 0 0 1 0 6H7M4 22h16" />
              </svg>
                ...
                <div className="mt-8 flex flex-col gap-5 lg:flex-row lg:items-center">
                  <div className="flex h-12 min-w-0 flex-1 overflow-hidden rounded-lg shadow-sm">
                    {assuntos.map((item, index) => {
                      const percentual = resumo.valorTotalEmCentavos
                        ? (item.valorEmCentavos / resumo.valorTotalEmCentavos) * 100
                        : 0;
                      return (
                        <div
                          key={item.nome}
                        ...
                        >
                          {percentual >= 4 ? `${percentual.toFixed(percentual >= 10 ? 0 : 1)}%` : ""}
                        </div>
                      );
                    })}
                  </div>
                  <div className="shrink-0 border-l-4 border-slate-700 pl-3">
                    <p className="text-[10px] font-semibold uppercase text-slate-500">Total</p>
                    <strong className="text-sm text-slate-800">{formatarMoeda(resumo.valorTotalEmCentavos)}</strong>
                  </div>
                </div>
                <div className="mt-8 grid gap-3 border-t border-slate-200 pt-5 md:grid-cols-2 xl:grid-cols-3">
                  {assuntos.map((item, index) => {
                    const percentual = resumo.valorTotalEmCentavos
                      ...
                      : 0;
                    return (
                      <div key={item.nome} className="flex items-center gap-3 rounded-lg bg-slate-50 px-3 py-3">
                        <span
                          className="h-8 w-1 shrink-0 rounded-full"
                          style={{ backgroundColor: coresAssuntos[index] }}
                        />
                        <span className="min-w-0 flex-1 truncate text-xs font-medium text-slate-700" title={item.nome}>
                          {item.nome}
                        </span>
                        <strong className="text-xs text-emerald-600">{formatarMoeda(item.valorEmCentavos)}</strong>
        ...
        <section id="emendas" className="mt-14" aria-labelledby="titulo-emendas">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 id="titulo-emendas" className="mt-2 text-2xl font-semibold text-[#12334d]">
                Emendas recentes
              </h2>
              <p className="mt-1 text-sm text-slate-500">Últimos registros disponibilizados no portal.</p>
            </div>
            <Link
              href={comLegislatura("/emendas", legislatura.slug)}
              className="text-sm font-semibold text-[#19689b] hover:underline"
            >
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                  ...
                  {emendas.map((emenda) => (
                    <tr key={emenda.id} className="transition hover:bg-slate-50/80">
                      <td className="max-w-[320px] px-5 py-4">
                        ...
                        <p className="mt-1 truncate text-xs text-slate-500">{emenda.beneficiarioFinal}</p>
                      </td>
                      <td className="px-5 py-4 text-sm text-slate-600">
                        {vereadoresPorId.get(emenda.vereadorId)?.nome ?? "Não identificado"}
                      </td>
            ...
            {!emendas.length && (
              <p className="px-6 py-12 text-center text-sm text-slate-500">Nenhuma emenda cadastrada.</p>
            )}
          </div>
        </section>
        ...
        <section
          id="vereadores"
          className="mt-14 rounded-3xl bg-[#eaf3f8] p-6 sm:p-10"
          aria-labelledby="titulo-vereadores"
        >
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div className="max-w-2xl">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#9a6c1d]">Atuação parlamentar</p>
              <h2 id="titulo-vereadores" className="mt-2 text-2xl font-semibold text-[#12334d]">
                Consulte por vereador
              </h2>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Veja quanto cada parlamentar indicou e consulte individualmente suas emendas.
              </p>
            </div>
            <Link
              href={comLegislatura("/vereadores", legislatura.slug)}
              className="inline-flex h-11 items-center justify-center rounded-xl bg-[#19689b] px-5 text-sm font-semibold text-white transition hover:bg-[#12577f]"
            >
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {destaques.map((item) => (
              <Link
                key={item.vereadorId}
                href={comLegislatura(`/vereadores/${item.vereadorId}`, legislatura.slug)}
                className="rounded-2xl bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-md"
              >
                <span className="relative flex h-14 w-14 overflow-hidden rounded-full bg-slate-200 ring-2 ring-white">
                  {item.vereador!.imagemUrl ? (
                    <Image
                      src={item.vereador!.imagemUrl}
                      alt={`Foto de ${item.vereador!.nome}`}
                      fill
                      sizes="56px"
                    ...
                    <svg
                      ...
                      stroke="currentColor"
                      strokeWidth="1.6"
                      className="m-auto h-7 w-7 text-slate-400"
                      aria-hidden="true"
                    >
